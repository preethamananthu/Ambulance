    ...
    loader.load().then(() => {
      ...
      if (mapElement) {
        ...
        originAutocomplete.addListener('place_changed', () => {
          ...
          updateRoute(place.formatted_address as string, destinationInputRef.current?.value || '');
        });

        // Listen for place changes in the destination input field
        destinationAutocomplete.addListener('place_changed', () => {
          const place = destinationAutocomplete.getPlace();
          updateRoute(originInputRef.current?.value || '', place.formatted_address as string);
        });
      }
    });
  }, []);

  const updateRoute = (origin:string, destination:string) => {
    const directionsService = new google.maps.DirectionsService();
    const directionsRenderer = new google.maps.DirectionsRenderer();
